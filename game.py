...
def initialise_board():
    board = [[0]*4 for _ in range(4)]           #making a 4X4 matrix filled with 0 and two 2s at
...
def merge_left(row):
    new_row=[i for i in row if i != 0]  #removing 0s
    for i in range(len(new_row) - 1):
        if new_row[i] == new_row[i+1]:
            new_row[i] *=2              #doubling the i'th term
            new_row[i+1] = 0            #making the i+1 st term 0
    new_row = [i for i in new_row if i !=0] #removing 0s
    return new_row + [0]*(4-len(new_row))   #filling the extra spaces with 0s
